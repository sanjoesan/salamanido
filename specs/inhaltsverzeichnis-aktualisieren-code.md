...
## 0.0 Nachverifikation am 2026-07-05 — Code-Drift seit Erst-Erstellung (verbindlich vor allen anderen Zeilenangaben)

Der übrige Text dieses Plans (Abschnitte 0.1 ff.) wurde am 2026-07-04 verfasst. Seither
ist der Code **nachweislich gewachsen** (v. a. durch das zwischenzeitlich umgesetzte
Feature „Ausschneiden"). Alle Zeilennummern **unterhalb dieses Abschnitts sind veraltet**
und wurden am 2026-07-05 erneut gegen den tatsächlichen Dateiinhalt geprüft. Diese Tabelle
ist die **maßgebliche** Referenz; wo sie einer Angabe weiter unten widerspricht, gilt
diese Tabelle. Die Architektur (Abschnitte 1–3), Datenmodell-Entscheidung und die
Reader/Writer-Strategie bleiben davon **unberührt und weiterhin gültig** — es ändern sich
nur Anker und einige konkret unten korrigierte Bedien-Details.

| Datei | Zeilen (jetzt) | Anker/Fakten am 2026-07-05 |
|---|---|---|
| `src/formats/shared/schema.ts` | 201 (Plan: 154) | `heading` **26–38** (nicht 19–31); `unsupported_block` 92–113; Listen 115–152; `tableNodes(...)` 154; Marks ab 157; `wordSchema` 198; `ProseMirrorJSON` **200–201** (nicht 153). `toc`/`toc_entry` sind **nach Zeile 38** (Ende `heading`) und **vor `text`** (Zeile 40) einzufügen. |
| `src/formats/shared/editor/commands.ts` | 167 (Plan: 108) | Exporte jetzt zusätzlich `insertHardBreak`, `insertTable`, `canCut`, `cutSelection`, `CutHandlers`, Re-Export `isInTable`. `import type { Command, EditorState } from 'prosemirror-state'` (Zeile 1) — **`Selection` und `EditorView` werden derzeit NICHT importiert** und müssen für Abschnitt 4.2 ergänzt werden (`import { Selection } from 'prosemirror-state'` — passend zum dortigen `Selection.near(...)`; alternativ `TextSelection.near` wie in `WordEditor.tsx:46`; `import type { EditorView } from 'prosemirror-view'`; `import { recomputeAllTablesOfContents, recomputeOneTableOfContents, findAllTocNodes, type JsonNode } from '../toc'`). `wordSchema` ist bereits importiert (Zeile 4). |
| `src/formats/shared/editor/Toolbar.tsx` | 297 (Plan: 247) | **`ToolbarProps` ist jetzt `{ view; cutError: string \| null; setCutError }`** (Zeilen 22–26) — **nicht** `{ view }`. `Toolbar({ view, cutError, setCutError })` Zeile 113. **SVG-Icon-Präzedenz existiert bereits**: `ScissorsIcon` (33–53), eingebettetes `<svg>` mit `stroke="currentColor"`. Cut-Button = erste Gruppe (143–156), inline `cutError`-Anzeige `role="alert"` (157–161), Trenner 163, Absatzformat-`<select>` 165–180, Marks 184–187, Farben 191–230, Ausrichtung 234–237, Listen 241–273, Trenner 275, Tabelle 277–289, Bild 291–294. Die neue ToC-Gruppe wird **nach Bild (Zeile 294)** als letzte Gruppe angehängt. |
| `src/formats/shared/editor/WordEditor.tsx` | 185 (Plan: 134) | **Erneut gedriftet gegenüber der 2026-07-04-Angabe (siehe Nachtrag 0.0.1 unten).** Keymap-Objekt jetzt **85–107** (nicht 77–99), enthält **zusätzlich** `Shift-Enter: insertHardBreak()` (**97**) und `Shift-Delete: cutSelection({ onCutBlocked: setCutError })` (**106**) — F9 ist **in dieses bestehende Objekt einzufügen**, nicht in ein verkürztes. **Wichtig: das transiente, sich selbst ausblendende Feedback-Muster wurde inzwischen in einen wiederverwendbaren Custom-Hook `useAutoDismiss(value, setValue, ms = 4000)` extrahiert** (Definition **52–63**, für Cut aufgerufen als `useAutoDismiss(cutError, setCutError)` Zeile **74**); `cutError`-State jetzt Zeile **71**. Zusatzbefund D (Abschnitt 0.5) ist damit weiterhin **überholt**, aber Korrektur K1 ist auf den Hook zu präzisieren (siehe K1 unten). `Toolbar`-Rendering mit `cutError`/`setCutError` Zeile **170**. `dispatchTransaction`→`onChange({ ...doc.content, body: newState.doc.toJSON() })` Zeile **129**. `TextSelection` wird bereits importiert (Zeile 2), `TextSelection.near(...)` in `reconcileSelectionOnClick` Zeile 46 (Anker für Abschnitt 4.2 unverändert korrekt). |
| `src/formats/shared/editor/pagination.ts` | 116 | `computePageBreakIndices` 12, `computePageCount` 27, `measureAndBuildDecorations` 33 (`doc.forEach` 43), `createPaginationPlugin` 72, `view()`-Hook/`update` **88–104**, `sameDecorationSet` 107. |
| `src/formats/docx/reader.ts` | 555 (Plan: 391) | `collectRuns` 194, `readBodyChildren` **464–485** (behandelt nur `w:p`/`w:tbl`), `readDocx` 487. |
| `src/formats/docx/writer.ts` | 318 (Plan: 280) | `blockToDocx` **105–155** (`switch` 111; `case 'heading'` **119–124**; `default: return ''` **153**); `writeDocx` **252**; Body-Zugriff `(doc.body as unknown as { content: JsonNode[] }).content` **Zeile 256**; `HEADING_STYLE_ID`/`headingStylesXml` importiert Zeile 6. **Kein `TableNameSequence`-Äquivalent** — ein ToC-Zähler ist neu durch `blockToDocx` zu fädeln (siehe Korrektur K4). |
| `src/formats/docx/styleDefs.ts` | 74 | `HEADING_STYLE_ID` 5–7, `headingStylesXml` 9. |
| `src/formats/odt/reader.ts` | 409 (Plan: 286) | `EMPTY_REDLINE_MARKER_NAMES` inkl. `bookmark`/`-start`/`-end` 84–86; `elementToBlocks` 250 (`text:h` 256–262, `text:section`-Entpacken 276–278, **`return []`-Fallthrough weiterhin Zeile 323** — Planangabe stimmt hier noch). |
| `src/formats/odt/writer.ts` | 305 (Plan: 211) | **`TableNameSequence` (Klasse Zeile 54, `.next()`)** = direkte Vorlage für einen ToC-Index-Zähler (K4); `blockToOdt` **85** (`case 'heading'` **93–98**; `default: return ''` 192); `writeOdt` **260**; Body-Zugriff `(doc.body as unknown as JsonNode).content` **266**; `blocksToOdt` fädelt `tableNames` durch (203). |
| `src/formats/odt/styleRegistry.ts` | 103 | `serializeDefs` 41, `paragraphAlignStyleDefs` 68, `headingStyleName` 80, `headingStyleDefs` 84–93. |
| `src/formats/shared/documentModel.ts` | 21 | `WordDocumentContent { body, header, footer, meta }` — kein ToC-Feld (unverändert). |
| `src/app/DocumentWorkspace.tsx` | 150 | **Zusatzbefund C bestätigt**: Export über `module.exportFile(snapshot.content, snapshot.fileName)` **Zeile 81** — reines `WordDocumentContent`-JSON, **keine** `EditorView`. Bestätigt die Architektur „reine JSON-Funktion in `toc.ts`, von Editor **und** Writer geteilt". |

### Substanzielle Korrekturen (nicht nur Zeilennummern)

- **K1 — Zusatzbefund D (Abschnitt 0.5) ist überholt; das Toast-Muster ist NICHT neu zu
  erfinden — inzwischen sogar als wiederverwendbarer Hook vorhanden.** `WordEditor.tsx`
  besitzt seit „Ausschneiden" das benötigte transiente, sich selbst ausblendende
  Rückmeldungsmuster; es wurde zwischenzeitlich (siehe Nachtrag 0.0.1) in einen **eigenen
  Custom-Hook** extrahiert:
  `function useAutoDismiss(value: string | null, setValue: (value: null) => void, ms = 4000)`
  (Definition Zeilen **52–63**), der intern genau den `useEffect`+`window.setTimeout`+
  `clearTimeout`-Auto-Dismiss kapselt und für Cut als `useAutoDismiss(cutError, setCutError)`
  (Zeile **74**) aufgerufen wird; `cutError`-State ist `useState<string | null>(null)`
  (Zeile **71**), an die Toolbar durchgereicht (Zeile 170), dort inline mit `role="alert"`
  gerendert (Toolbar 157–161). **Die ToC-Rückmeldung ist über GENAU DIESEN vorhandenen Hook
  zu bauen — NICHT** ein zweites, handgeschriebenes `useEffect` daneben (das wäre die
  duplizierte Variante, die K1 gerade vermeiden will) und **nicht** der in der Rohfassung
  von Abschnitt 6.2 skizzierte manuelle `useRef`+`setTimeout`-Ansatz. Konkret:
  `const [tocFeedback, setTocFeedback] = useState<TocUpdateOutcome | null>(null)` gefolgt von
  `useAutoDismiss(tocFeedback, setTocFeedback, 2500)`. **Typkompatibilität ist gegeben**, weil
  `TocUpdateOutcome` (`'not-found' | 'already-current' | 'updated'`) ein String-Union-Subtyp
  von `string` ist: `TocUpdateOutcome | null` ist `string | null` zuweisbar, und der
  React-Setter ist mit `(value: null) => void` kontravariant kompatibel. Siehe korrigiertes
  Abschnitt 6.2. Damit entfällt der in Abschnitt 0.5 behauptete Neubau-Aufwand ersatzlos —
  es genügt **ein** zusätzlicher `useState` plus **ein** Hook-Aufruf.
- **K2 — `ToolbarProps` wird ERWEITERT, nicht ersetzt.** Der Patch in Abschnitt 5.1
  (`export function Toolbar({ view, onTocFeedback }: ToolbarProps)`) würde die
  vorhandenen `cutError`/`setCutError`-Props löschen und den Cut-Button brechen. Korrekt:
  `interface ToolbarProps { view; cutError; setCutError; onTocFeedback?: (o:
  TocUpdateOutcome) => void }` und `Toolbar({ view, cutError, setCutError, onTocFeedback })`.
- **K3 — Interne Flip-Flops im Plan sind aufzulösen (einmalige Endentscheidung):**
  - *`w:sdt`-Wrapper (Abschnitt 9.1 vs. 9.2):* **Endgültig KEIN `w:sdt` beim Schreiben.**
    Der Writer verwendet das klassische `w:fldChar`/`w:instrText`-Tripel, umklammert von
    `w:bookmarkStart`/`w:bookmarkEnd` (`SalamanidoTocN`) als Container-Marker (Abschnitt
    9.2 „Endgültige Struktur", der zweite `tocBlockToDocx`-Codeblock). Der **erste**
    Codeblock in 9.2 (mit `<w:sdt><w:sdtContent>`) ist ein verworfener Zwischenstand und
    beim Implementieren zu ignorieren. Der **Reader** erkennt `w:sdt`-Varianten trotzdem
    zusätzlich (Import realer Word-Dateien, Abschnitt 9.4).
  - *`cachedPage`-Attribut (Abschnitt 8.1 vs. 9.2/12):* **Standardempfehlung: WEGLASSEN.**
    Der `cachedPage`-Cache-Erhalt-Mechanismus ist der komplexeste, fragilste Einzelpunkt
    des Plans (Abschnitt 9.2 gesteht das selbst ein, R3). Anforderung 3.7 erlaubt
    ausdrücklich einen „plausiblen, zuletzt berechneten Platzhalterwert" — Word/LibreOffice
    berechnen die Seitenzahl beim Öffnen ohnehin neu. **Empfohlener Default:** **kein**
    `toc_entry.cachedPage`-Schema-Attribut; der Writer emittiert für die Seitenzahl den
    `PAGEREF`-Feld-Platzhalter (DOCX, von Word gefüllt) bzw. eine live-Näherung/`"1"` (ODT).
    Die Schema-Tabelle in Abschnitt 2 bleibt damit bei **zwei** neuen Nodes ohne
    `cachedPage`. Wer die bessere Baseline-Treue will, kann `cachedPage` optional
    nachrüsten — es ist **kein** Abnahme-Blocker (deckt sich mit Abschnitt 16 Punkt 5).
    Die Übersichtstabelle Abschnitt 12 ist entsprechend zu lesen (Eintrag „inkl.
    `toc_entry.cachedPage`" ist optional/gestrichen).
- **K4 — ToC-Index-Sequenzierung ist konkret zu fädeln.** `tocBlockToDocx`/`tocBlockToOdt`
  brauchen einen pro Dokument eindeutigen `tocIndex` (für `SalamanidoTocN`, Grenzfall 3).
  Für ODT existiert die **exakte Vorlage** bereits: `TableNameSequence` (`odt/writer.ts:54`,
  `.next()`), durch `blocksToOdt(... tableNames)` gefädelt — analog eine `TocNameSequence`
  anlegen und mitfädeln. Für DOCX (kein solcher Mechanismus) einen einfachen Zähler analog
  zu `images`/`rels` durch `blockToDocx` reichen. Ohne dies wäre `tocIndex` in Abschnitt
  9.2/11.1 undefiniert.
- **K5 — SVG-Icon: Präzedenz nutzen.** Abschnitt 5.1s `TocUpdateIcon` ist korrekt als
  eingebettetes SVG; es folgt exakt dem bereits vorhandenen `ScissorsIcon`
  (`Toolbar.tsx:33–53`, `stroke="currentColor"`, `aria-hidden`) — dieselbe Konvention
  verwenden (Attribut `strokeWidth` als camelCase-JSX-Prop, `focusable="false"`).
- **K6 — `default`-Fall der Writer nicht vergessen.** `blockToDocx`/`blockToOdt` haben
  `default: return ''` (DOCX 153, ODT 192). Solange der `case 'toc'` fehlt, wird ein
  `toc`-Knoten **still verworfen** — der neue `case 'toc'` ist also nicht nur für die
  Funktion, sondern auch als Regression-Schutz gegen stillen Verlust zwingend (Anforderung
  Abschnitt 18). Ein isoliert (ohne umschließendes `doc`) durch den Writer laufender
  `toc_entry` trifft ebenfalls `default` — unkritisch, da Einträge stets innerhalb
  `tocBlockTo*` gerendert werden, nie als Top-Level-`case`.

### 0.0.1 Zweite Nachverifikation (später am 2026-07-05) — `WordEditor.tsx` erneut refaktoriert

Bei einer nochmaligen Direktdurchsicht aller in Abschnitt 0.0 genannten Dateien am selben
Tag ist **nur eine** Datei gegenüber der obigen Tabelle weiter gedriftet: `WordEditor.tsx`
(jetzt **185** Zeilen). Alle anderen Anker der 0.0-Tabelle (`schema.ts`, `commands.ts`,
`Toolbar.tsx`, `pagination.ts`, `docx/writer.ts` inkl. `blockToDocx` 105–155/`case 'heading'`
119–124/`default` 153, `odt/writer.ts` inkl. `TableNameSequence` 54/`blockToOdt` 85/`case
'heading'` 93–98/`default` 192, beide Reader) wurden **erneut punktweise bestätigt** und
bleiben gültig. In `WordEditor.tsx` wurde das transiente Feedback in den wiederverwendbaren
Hook `useAutoDismiss` (52–63) ausgelagert; dadurch sind sämtliche Zeilenangaben der
`WordEditor.tsx`-Zeile in Abschnitt 0.0 um ~8 Zeilen nach unten gewandert. Die **oben bereits
korrigierte** `WordEditor.tsx`-Tabellenzeile und Korrektur K1 tragen dem Rechnung; maßgeblich
sind die dortigen aktualisierten Anker (Keymap 85–107, `Shift-Enter` 97, `Shift-Delete` 106,
`useAutoDismiss` 52–63/Aufruf 74, `cutError`-State 71, `Toolbar` 170, `dispatchTransaction`
129). **Lehre für die Umsetzung:** vor dem eigentlichen Editieren jede Zeilenangabe ein
letztes Mal direkt gegen den Dateiinhalt prüfen (Anker driften bei jedem zwischenzeitlich
gemergten Feature) — die Architektur (Abschnitte 1–3, Datenmodell, Reader/Writer-Strategie)
ist davon unberührt.
...
### 0.5 Zusatzbefund D: ~~kein bestehendes Transient-Feedback-/Toast-Muster~~ — ÜBERHOLT (siehe Korrektur K1 in Abschnitt 0.0)

**Dieser Befund war am 2026-07-04 zutreffend, ist seit 2026-07-05 aber überholt.** Inzwischen
existiert in `WordEditor.tsx` ein transientes, sich selbst ausblendendes Rückmeldungsmuster,
das (siehe Nachtrag 0.0.1) zusätzlich in einen **wiederverwendbaren Custom-Hook**
`useAutoDismiss(value, setValue, ms = 4000)` extrahiert wurde (Definition Zeilen **52–63**,
für Cut aufgerufen als `useAutoDismiss(cutError, setCutError)` Zeile **74**; `cutError`-State
Zeile **71**; an die Toolbar durchgereicht Zeile **170**, dort inline mit `role="alert"`
gerendert, Toolbar 157–161). Die ToC-Rückmeldung ist **über genau diesen Hook**
wiederzuverwenden — kein Neubau eines Toast-Systems, kein zweites handgeschriebenes
`useEffect`, kein manueller `useRef`+`setTimeout`. Details im korrigierten Abschnitt 6.2.

*(Historischer Wortlaut, nur zur Nachvollziehbarkeit:* `PrivacyBanner.tsx` ist eine
dauerhafte Leiste; `PrivacyModal.tsx` ein blockierender Dialog; ein transientes Muster
gab es damals noch nicht. Das trifft **nicht mehr** zu.)
...
**Korrektur K2 (Abschnitt 0.0):** `ToolbarProps` ist aktuell `{ view; cutError: string |
null; setCutError }` (Toolbar.tsx:22–26) — der Button-Patch **erweitert** diese Props, er
ersetzt sie nicht (sonst bricht der Cut-Button). Die neue Gruppe wird **nach dem Bild-Label
(Toolbar.tsx:294)** als letzte Gruppe angehängt.
...
interface ToolbarProps {
  ...
  onTocFeedback?: (outcome: TocUpdateOutcome) => void
}

export function Toolbar({ view, cutError, setCutError, onTocFeedback }: ToolbarProps) {
  ...
  return (
    <div role="toolbar" aria-label="Textformatierung" className="...">
      {/* ... bestehende Gruppen (Cut, Absatzformat, Marks, Farben, Ausrichtung, Listen, Tabelle, Bild) unverändert ... */}
...
**Korrektur (Abschnitt 0.0/0.0.1):** F9 wird in das **bestehende** Keymap-Objekt
(WordEditor.tsx **85–107**, nicht 77–99) eingefügt, das inzwischen zusätzlich
`Shift-Enter: insertHardBreak()` (**97**) und
`Shift-Delete: cutSelection({ onCutBlocked: setCutError })` (**106**) enthält — diese
Bindungen **nicht** entfernen. Nur der eine `F9:`-Eintrag kommt hinzu:
...
keymap({
  ...
  'Shift-Enter': insertHardBreak(),                 // bereits vorhanden — beibehalten
  'Mod-b': toggleMark(wordSchema.marks.strong),
  'Mod-i': toggleMark(wordSchema.marks.em),
  'Mod-u': toggleMark(wordSchema.marks.underline),
  'Shift-Delete': cutSelection({ onCutBlocked: setCutError }), // bereits vorhanden — beibehalten
  F9: (_state, _dispatch, view) => {                // NEU
...
### 6.2 Feedback-State + minimale, sich selbst ausblendende Anzeige (über den vorhandenen `useAutoDismiss`-Hook, Korrektur K1)

**Korrektur K1 (Abschnitt 0.0/0.0.1):** **kein** zweites, handgeschriebenes `useEffect`
danebensetzen — den seit „Ausschneiden" vorhandenen, inzwischen als **wiederverwendbarer
Custom-Hook** extrahierten Auto-Dismiss-Mechanismus direkt wiederverwenden
(`useAutoDismiss(value, setValue, ms = 4000)`, `WordEditor.tsx:52–63`, für Cut bereits
`useAutoDismiss(cutError, setCutError)` Zeile 74). So bleibt genau **ein**
Timer-Lifecycle-Muster im Editor statt zweier verschiedener, und die ToC-Rückmeldung
kostet nur einen `useState` plus einen Hook-Aufruf:

```ts
// Neben dem vorhandenen `const [cutError, setCutError] = useState<string | null>(null)` (Zeile 71):
const [tocFeedback, setTocFeedback] = useState<TocUpdateOutcome | null>(null)

// Denselben vorhandenen Hook wiederverwenden — KEIN eigenes useEffect danebensetzen.
// `TocUpdateOutcome` ist ein String-Union-Subtyp von `string`, deshalb typkompatibel
// mit `useAutoDismiss(value: string | null, setValue: (value: null) => void, ms)`:
useAutoDismiss(tocFeedback, setTocFeedback, 2500)
```

Rendering (innerhalb des `return`; die Toolbar-Zeile ist aktuell Zeile 170 und reicht
bereits `cutError`/`setCutError` durch — `onTocFeedback` wird **zusätzlich** angehängt):

```tsx
{viewRef.current && (
  <Toolbar view={viewRef.current} cutError={cutError} setCutError={setCutError} onTocFeedback={setTocFeedback} />
)}
{tocFeedback && (
...
`role="status"`/`aria-live="polite"` ist eine bewusst **transiente** Nachricht; die
inline-`role="alert"`-Anzeige des Cut-Fehlers bleibt davon unberührt (zwei getrennte,
unabhängige Rückmeldungskanäle). Der F9-Pfad (6.1) und der Toolbar-Klick-Pfad (5.1) rufen
beide `setTocFeedback` — der Toolbar-Pfad über den `onTocFeedback`-Prop, der F9-Pfad direkt
(F9 lebt in `WordEditor.tsx`, hat also `setTocFeedback` im Scope).
...
|---|---|---|
| `src/formats/shared/schema.ts` | geändert | `heading.headingId`-Attribut; neue Nodes `toc`, `toc_entry`. **`toc_entry.cachedPage` per Korrektur K3 (Abschnitt 0.0) standardmäßig WEGLASSEN** (optional, kein Abnahme-Blocker) |
