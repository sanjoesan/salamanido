...
|---|---|---|
| Klick markiert Bild als `NodeSelection` (interner Zustand) | ProseMirror-Standardverhalten (`selectable`-Default), bereits vorhanden | **Ja** |
| Entf/Rücktaste löscht ein selektiertes Bild | `prosemirror-commands`s `baseKeymap` (`deleteSelection`), bereits vorhanden, **keine Code-Änderung nötig** | **Ja** — sofort testbar, siehe §5.3 TF1b–TF9 |
...
| Basis-Löschen (ein Bild, Klick + Entf) | E2E | `tests/e2e/image-delete.spec.ts` TF1b |
...
### 2.4 Karten-Locator und Bild-Einfüge-Helper
...
/** Echter filechooser-Flow (setInputFiles auf das versteckte input[type=file]), kein
 *  interner insertImage()-Aufruf — deckt sich mit dem in `FEATURE-SPEC-DOCX-ODT.md` §7
 *  Testfall 1 geforderten echten Einfüge-Weg. */
async function insertImage(card: ReturnType<typeof docxCard>, buffer: Buffer, name: string) {
  const fileInput = card.locator('input[type="file"][accept="image/*"]')
...
import JSZip from 'jszip'
import { RED_PNG_BASE64, GREEN_PNG_BASE64, BLUE_PNG_BASE64, pngBuffer, makeLargePng } from './fixtures/testImages'
...
async function insertImage(card: ReturnType<typeof docxCard>, buffer: Buffer, name = 'test.png') {
  await card.locator('input[type="file"][accept="image/*"]').setInputFiles({ name, mimeType: 'image/png', buffer })
}
...
test('TF1a (Pflicht-Vorbedingung, Req DoD 1): ein selektiertes Bild zeigt einen tatsächlich sichtbaren Rahmen', async ({ page }) => {
  ...
  await editor.click()
  await insertImage(docxCard(page), pngBuffer(RED_PNG_BASE64))
...
**TF1b — Basis-Löschen (heute schon lauffähig, unabhängig von TF1a)**

```ts
test('TF1b: Bild einfügen, anklicken, Entf löscht es vollständig', async ({ page }) => {
...
test('TF3: Bild löschen, dann Strg+Z stellt es mit identischem Alt-Text wieder her', async ({ page }) => {
  ...
  const editor = page.locator('.ProseMirror')
  await editor.click()
  await insertImage(docxCard(page), pngBuffer(RED_PNG_BASE64), 'original-datei.png')
...
test('TF5: drei farblich unterschiedliche Bilder, mittleres per Klick löschen — Rest bleibt unverändert und unterscheidbar', async ({ page }) => {
  ...
  const editor = page.locator('.ProseMirror')
  await editor.click()
  await insertImage(docxCard(page), pngBuffer(RED_PNG_BASE64), 'rot.png')
  await page.keyboard.press('Enter')
  await insertImage(docxCard(page), pngBuffer(GREEN_PNG_BASE64), 'gruen.png')
  await page.keyboard.press('Enter')
  await insertImage(docxCard(page), pngBuffer(BLUE_PNG_BASE64), 'blau.png')

  const images = editor.locator('img')
  await expect(images).toHaveCount(3)
  const srcsBefore = await images.evaluateAll((els) => els.map((el) => (el as HTMLImageElement).src))

  await images.nth(1).click() // grünes, mittleres Bild
  await page.keyboard.press('Delete')
...
test('TF6: Bild in einer Tabellenzelle löschen — nur das Bild verschwindet, Tabellenstruktur bleibt', async ({ page }) => {
  ...
  await cells.nth(0).locator('img').click()
  await page.keyboard.press('Delete')
...
test('TF7 (PFLICHT): Bild selektieren → Klick zur Neupositionierung → Enter → weiter tippen bleibt korrekt', async ({ page }) => {
  ...
  await editor.locator('img').click() // erzeugt NodeSelection auf das Bild
...
test('TF13: mehrere-MB-Bild einfügen und löschen — kein Einfrieren, kein Konsolenfehler', async ({ page }) => {
  ...
  await insertImage(docxCard(page), makeLargePng(1200), 'gross.png')
...
test('TF14 (Pflicht, Grenzfall 9): Bild in einer Zelle der inneren Tabelle löschen — kein Absturz, beide Ebenen bleiben konsistent', async ({ page }) => {
  ...
  await innerCells.nth(0).locator('img').click()
  await page.keyboard.press('Delete')
...
test('TF15: nach dem Löschen ersetzt ein neu eingefügtes Bild die Position ohne Attribut-Vermischung', async ({ page }) => {
  ...
  const editor = page.locator('.ProseMirror')
  await editor.click()
  await insertImage(docxCard(page), pngBuffer(RED_PNG_BASE64), 'altes-bild.png')
  await editor.locator('img').click()
  await page.keyboard.press('Delete')
  await expect(editor.locator('img')).toHaveCount(0)

  await insertImage(docxCard(page), pngBuffer(BLUE_PNG_BASE64), 'neues-bild.png')
...
**Zusatztest — Grenzfall 8: Bild als einziger Inhalt eines Listenpunkts**

```ts
test('Grenzfall 8: Bild als einziger Listenpunkt-Inhalt löschen lässt den (leeren) Listenpunkt bestehen', async ({ page }) => {
  await docxCard(page).getByRole('button', { name: 'Neu erstellen' }).click()
  const editor = page.locator('.ProseMirror')
  await editor.click()
...
test('RT2 (Req 4.2 Testfall 2): ODT — identische Sequenz, gegen manifest.xml/Pictures/ geprüft', async ({ page }) => {
  ...
  await page.keyboard.type('Text davor.')
  await page.keyboard.press('Enter')
  await insertImage(odtCard(page), pngBuffer(RED_PNG_BASE64))
  await page.keyboard.press('Enter')
  await page.keyboard.type('Text danach.')
  ...
  const input = odtCard(page).locator('input[type="file"]').first()
  await input.setInputFiles({ name: 'reimport.odt', mimeType: 'application/vnd.oasis.opendocument.text', buffer: exportedBuffer })
  await expect(page.locator('.ProseMirror img')).toHaveCount(0)
})
...
test('RT3 (Req 4.2 Testfall 3): mehrere Bilder, eines löschen → Export → Reimport → genau das erwartete Bild fehlt', async ({ page }) => {
  ...
  await editor.locator('img').nth(0).click()
  await page.keyboard.press('Delete')
...
test('RT6 (Req 4.2 Testfall 6): alle Bilder nacheinander löschen → Export → Reimport → keine Bild-Referenzen mehr', async ({ page }) => {
  ...
  await insertImage(docxCard(page), pngBuffer(RED_PNG_BASE64), 'a.png')
  await insertImage(docxCard(page), pngBuffer(GREEN_PNG_BASE64), 'b.png')
  await insertImage(docxCard(page), pngBuffer(BLUE_PNG_BASE64), 'c.png')
...
test('RT7 (Req 4.2 Testfall 7): Cross-Format — ODT importieren, Bild löschen, als DOCX exportieren, reimportieren', async ({ page }) => {
  ...
  const input = odtCard(page).locator('input[type="file"]').first()
  await input.setInputFiles({ name: 'reimport.odt', mimeType: 'application/vnd.oasis.opendocument.text', buffer: odtBuffer })
  await expect(page.locator('.ProseMirror img')).toHaveCount(0)
...
| §1 Zugriffsweg 1 — Klick + Entf | E2E | TF1b | lauffähig |
...
| Grenzfall 8 | E2E | Zusatztest „Grenzfall 8“ | lauffähig |
...
7. Abschließend: gesamte Suite (`npm test && npm run test:e2e`) für den finalen QA-Sign-off
   gemäß §7.
