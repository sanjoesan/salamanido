...
`state`-Closure-Variable und dispatcht jede einzeln; `WordEditor.tsx`s
`dispatchTransaction` wendet jede über `view.state.apply(tr)` auf das inzwischen
bereits veränderte `view.state` an. Ab dem zweiten betroffenen Block wirft
`EditorState.apply()` `RangeError: "Applying a mismatched transaction"` — unbehandelt,
kein `try`/`catch`, keine Error-Boundary. **Strg+A → Blocksatz auf ein
Mehrfach-Absatz-Dokument ist damit real defekt**, exakt wie vermutet.

Zusätzlich beim Audit gefunden, **über den in der Anforderung explizit benannten
Verdacht hinausgehend**:

1. Selbst nach einem naheliegenden Fix („eine Transaction, mehrere
   ...
   `from`/`to`-Intervall — sonst wird bei einer nicht-rechteckigen `CellSelection`
   (z. B. Spalte 2 von 3 markiert) auch die dazwischenliegende, **nicht markierte**
   Zelle 3 mitjustiert. Das wäre kein Absturz, sondern ein **stiller
   Korrektheitsfehler** (falsche Zellen verändert) — schlimmer als die Exception, weil
   unbemerkt. Beide Fehler werden mit derselben Code-Änderung behoben (Abschnitt 3.1).
2. `setHeading` (`commands.ts:40-55`) setzt `align` beim Formatvorlagenwechsel hart auf
   `'left'` zurück — bestätigt exakt wie in der Anforderung beschrieben. **Entscheidung
   getroffen** (Abschnitt 2.5 der Anforderung verlangt eine Entscheidung, keine bloße
...
3. `isAlignActive` (`commands.ts:29-38`) liest nur `$from` — bestätigt. **Entscheidung
   getroffen** (Anforderung 2.3/Grenzfall 3.4 verlangt Klärung): bei einer
   ...
   erste.
4. Kein Toggle-Charakter, aber die Anforderung (2.4) bittet als Nice-to-have um
...
5. `Toolbar.tsx`s `AlignButton` hat kein `aria-label` — bestätigt, wird gefixt
   (betrifft alle vier Buttons gleichermaßen).
6. Kein Tastenkürzel — bestätigt. **Entscheidung**: wird ergänzt, aber **bewusst
   nicht** als `Strg+L/E/R/J` (Word-Konvention), weil `Strg+L` (Adressleiste) und
   `Strg+J` (Downloads-Ablage) in Chrome/Edge auf Windows/Linux browser-reservierte
   Tastenkombinationen sind, die die Seite nie zu Gesicht bekommt — siehe Abschnitt 3.8
   für die vollständige Risikoabwägung und die gewählte Alternative.
7. `docx/reader.ts`s `JC_TO_ALIGN` und `odt/reader.ts`s rohe Übernahme von
   `fo:text-align` kennen `start`/`end` nicht (Grenzfall 8/9 der Anforderung) — **mit
   echten Fixture-Dateien aus dem vorhandenen Korpus bestätigt** (nicht nur
   spekuliert, siehe Abschnitt 6): `rtl.docx`, `table-alignment.docx`,
   `table-indent.docx`, `unicode-path.docx` (DOCX, `w:jc val="start"`) sowie 14
   ODT-Fixtures inkl. `feature_attributes_paragraph_MSO2013.odt`
   (`fo:text-align="end"`). Wird gefixt (LTR-Näherung `start→left`, `end→right`,
   dokumentierte Grenze siehe Abschnitt 3.9/3.11).
8. **Korrektur an der Anforderung selbst, durch Fixture-Inspektion gefunden:**
   ...
   Abschnitt 0/Testfall 5.14 als bisher ungenutzte Ausrichtungs-Fixtures benannt —
   enthalten **keine einzige** horizontale Absatzausrichtung (`w:jc`/`fo:text-align`).
   Ersteres testet ausschließlich vertikale Zellausrichtung (`w:vAlign`, ein separates,
   nicht implementiertes Feature), letzteres ausschließlich `table:align="margins"`
   (Tabellenposition auf der Seite). Testfall 5.14 wird entsprechend korrigiert
   (Abschnitt 6).
...
der Übergabepunkte in Abschnitt 2.5), `specs/ausrichtung-{links,zentriert,rechts}-req.md`
...
(`CellSelection`, `dist/index.cjs` Zeile 517–523: der Konstruktor baut `ranges` als ein
`SelectionRange` je selektierter Zelle und übergibt sie an den `Selection`-Basiskonstruktor)
und `prosemirror-transform`s `setNodeAttribute` im Quellcode geprüft, um den Fix aus
Abschnitt 3.1 auf Bibliotheksebene abzusichern statt zu vermuten.
...
`fo:text-align`-Werten durchsucht** — Ergebnis vollständig in Abschnitt 6. Das ist
derselbe empirische Ansatz, den `durchgestrichen-code.md` Abschnitt 6 für
`text-line-through-style` verwendet hat, hier erstmals auch für DOCX vollständig
durchgeführt.
...
| `schema.ts:9-17`/`19-31` `paragraph`/`heading`, `parseDOM` liest `dom.style.textAlign \|\| 'left'`, `toDOM` rendert `text-align` | Ja, exakt so, für beide Node-Typen identisch | keine |
| `commands.ts:8,10` `Align`-Typ, `alignableTypes` | Ja, exakt so | keine |
| `commands.ts:13-27` `setAlign`: `nodesBetween(from,to,…)`, `dispatch(state.tr.setNodeAttribute(...))` je Treffer, `state` bleibt die ursprüngliche Closure-Variable | Ja, exakt so — **RangeError-Verdacht bestätigt**, siehe Abschnitt 3.1 | **bestätigter kritischer Bug** |
| `commands.ts:29-38` `isAlignActive`: nur `$from` | Ja, exakt so | bestätigt, siehe Abschnitt 3.4 |
| `commands.ts:40-55` `setHeading`: `align:'left'` hart bei Wechsel zu Überschrift; `attrs: undefined` (⇒ Schema-Default `left`) bei Wechsel zu Standard | Ja, exakt so | bestätigt, siehe Abschnitt 3.5 |
| `Toolbar.tsx:64-84` `AlignButton`: `title`, `aria-pressed`, **kein** `aria-label` | Ja, exakt so | bestätigt, siehe Abschnitt 3.6 |
| `Toolbar.tsx:185-188` vier Buttons, reine Unicode-Glyphen `⇤ ↔ ⇥ ≡` | Ja, exakt so | bestätigt, siehe Abschnitt 3.7 |
| `WordEditor.tsx:71-79` Keymap ohne Ausrichtungs-Kürzel | Ja, exakt so — nur `Mod-z/y/Shift-z`, `Enter`, `Mod-b/i/u` | bestätigt, siehe Abschnitt 3.8 |
| `WordEditor.tsx:91-98(99)` `dispatchTransaction` wendet `tr` auf aktuelles `view.state` an | Ja, exakt so (`view.state.apply(tr)`, dann `view.updateState(newState)`) | bestätigt — genau der Mechanismus, der Abschnitt 3.1 in Kombination mit `commands.ts:21` zum Absturz bringt |
| `docx/reader.ts:13` `JC_TO_ALIGN` kennt nur `left/center/right/both` | Ja, exakt so | bestätigt **und mit echten Fixtures belegt**, siehe Abschnitt 3.9/6 |
| `docx/reader.ts:150-152` liest nur direktes `w:pPr`, kein Style-`w:jc` | Ja, exakt so | bestätigt **und mit echter, sich selbst dokumentierender Fixture belegt**, siehe Abschnitt 3.13/6 |
| `docx/reader.ts:236-238` Tabellenzellen laufen durch `paragraphToBlocks` | Ja, exakt so | keine — bereits korrekt, siehe Abschnitt 3.14 |
| `docx/writer.ts:16` `JC_BY_ALIGN` | Ja, exakt so | keine |
| `docx/writer.ts:67-70` `paragraphPropsXml`, immer explizites `<w:jc>` | Ja, exakt so | bewusst unverändert, siehe Abschnitt 3.10 |
| `odt/reader.ts:24,63-65` `paragraphAligns` liest `fo:text-align` roh | Ja, exakt so (Zeile 64: `props?.getAttributeNS(fo,'text-align')`, Zeile 65: `if (align) paragraphAligns.set(name, align)`) | bestätigt **und mit echten Fixtures belegt**, siehe Abschnitt 3.11/6 |
| `odt/reader.ts:126,173` Fallback `'left'` | Ja, exakt so | keine, bleibt korrekt nach Fix |
| `odt/writer.ts:64-65,71-73` `PARAGRAPH_ALIGN_STYLE_NAME[align] ?? …left` | Ja, exakt so | keine, siehe Abschnitt 3.12 |
...
| `docx/__tests__/roundtrip.test.ts:48-53`, `odt/…` analog | Ja, `it.each([...])` mit `paragraph('Text', align)` — je Testlauf **ein** Absatz | bestätigt, siehe Abschnitt 5.1 |
...
- **`prosemirror-tables`s `CellSelection.ranges`** (Abschnitt 1): Das rohe
  ...
  Spalten-`CellSelection` mehr Text ab als tatsächlich selektiert ist — ein zweiter,
  unabhängiger Fehler, der auch nach einem naiven Exception-Fix bestehen bliebe
  (Abschnitt 3.1).
- **Testfall-Korrektur** (Abschnitt 0 Punkt 8, ausführlich Abschnitt 6):
  `TestTableCellAlign.docx`/`tabelleAlignMargin.odt` enthalten keine horizontale
  Ausrichtung; die Anforderung selbst listet sie fälschlich als einschlägig.
...
### 3.1 KERNBUG (Anforderung 2.2/5.1): `setAlign` wirft `RangeError` ab dem zweiten Block — **plus ein zweiter, unabhängiger Korrektheitsfehler bei `CellSelection`**
...
**Fehler 1 (der in der Anforderung vermutete, hier durch Codelektüre bestätigte):**
...
(`state.tr` — Zeile 21). `WordEditor.tsx`s `dispatchTransaction` (Zeile 91-93)
wendet jede über `view.state.apply(tr)` auf das zu diesem Zeitpunkt **aktuelle**
`view.state` an. Nach dem ersten `dispatch()` weicht `view.state.doc` (erster Block
bereits `justify`) von `tr.before` der zweiten, aus dem alten `state` gebauten
Transaction ab — `EditorState.apply()` verlangt Übereinstimmung und wirft
...
`Toolbar.tsx`s `run()`, keine React-Error-Boundary im `src`-Baum — geprüft, keine
vorhanden).

**Fehler 2 (neuer Fund, geht über die Anforderung hinaus):** Selbst nach einem
naheliegenden „eine Transaction, mehrere `setNodeAttribute`“-Fix bliebe das rohe
`{ from, to } = state.selection`-Intervall problematisch. Bei einer normalen
`TextSelection`/`AllSelection` ist `from`/`to` korrekt (ein zusammenhängender
Bereich). Bei einer `CellSelection` aus `prosemirror-tables` (Anforderung 2.6,
Grenzfall 4) ist `.from`/`.to` jedoch nur die **äußere Umschließung** der
...
Anforderung 2.4 (kein unnötiger Undo-Schritt bei bereits gesetzter Ausrichtung —
siehe Abschnitt 3.3 für die Begründung, hier bereits eingearbeitet):**
 ...
 * selection dispatches nothing (Anforderung 2.4: kein unnötiger Undo-Schritt). */
export function setAlign(align: Align): Command {
  return (state, dispatch) => {
...
- Rückgabewert bleibt `true`, auch wenn `positions.size === 0` (bereits überall
  ...
  Keymap-Chaining verwendet).
- Kein Toggle: ein Klick auf „Blocksatz“ setzt **immer** `justify`, nie zurück auf
  einen vorherigen Wert — exakt wie in Anforderung 2.1/2.4 gefordert (Ausrichtung
...
### 3.3 Anforderung 2.3/Grenzfall 3.4 — Anzeige bei Mehrfachselektion mit gemischter Ausrichtung
...
**Entscheidung (verbindliche Anzeige-Konvention für Anforderung 2.3/Grenzfall 3.4):**
...
export function isAlignActive(state: EditorState, align: Align): boolean {
  ...
  // gedrückt — siehe Anforderung 2.3/Grenzfall 3.4.
  let found = false
  let uniform = true
...
### 3.4 Anforderung 2.4 — Kein Toggle, aber Undo-Sparsamkeit
...
sind **beide** Teilfragen aus Anforderung 2.4 explizit beantwortet:
...
### 3.5 Anforderung 2.5 — `setHeading` setzt `align` beim Formatvorlagenwechsel zurück (geteilt mit `absatzformat-dropdown-req.md`)
...
zurück auf `'left'` (explizit bei Wechsel zu einer Überschrift, implizit über den
Schema-Default bei Rückwechsel zu Standard). Ein zuvor im Blocksatz stehender
Absatz verliert diesen beim Wechsel zu „Überschrift 1“ kommentarlos.

**Entscheidung (verbindlich, beantwortet Anforderung 2.5 und
`absatzformat-dropdown-req.md` Befund 4/Grenzfall 8-9 gemeinsam):** Ausrichtung
ist Direktformatierung (Zeichen-/Absatzattribut, unabhängig vom Node-Typ) und
...
export function setHeading(level: number | null): Command {
  return (state, dispatch) => {
    ...
    // Abschnitt 3.5 / Anforderung 2.5, geteilt mit absatzformat-dropdown-req.md
...
auf einen einzelnen Block, `!$from.sameParent($to)` verweigert Mehrfachselektion
komplett) — das ist eine eigenständige Design-Frage jener Anforderung
(Abschnitt 2.3 dort), nicht Gegenstand dieses Plans; hier nur insofern relevant,
als sie **nicht** durch den vorliegenden Fix verändert wird.

### 3.6 Anforderung 1 Zeile 4 / Bedienelement — fehlendes `aria-label`

`Toolbar.tsx:64-84`. Betrifft, wie bereits im TL;DR benannt, alle vier
`AlignButton`-Instanzen gleichermaßen, da sie dieselbe Komponente teilen. Fix,
zugleich Auflösung von Anforderung 1 Zeile 3 (sichtbarer deutscher Text statt nur
des internen `align`-Bezeichners im Tooltip — bisher `title="Ausrichtung: justify"`,
der interne englische Wert landet im Tooltip statt eines deutschen Begriffs):
...
jetzt intern aus `ALIGN_GLYPHS` abgeleitet):
...
**Präzisierung zur Anforderungstabelle Zeile 4:** Die Anforderung vermutet, `title`
werde „nur als Fallback benutzt, wenn kein Text-Inhalt vorhanden ist“ — das
betrifft nur das Fehlen von `aria-label`. Sobald `aria-label` gesetzt ist (wie
hier), hat es laut Accessible-Name-Berechnung (WAI-ARIA `accname`) **Vorrang**
...
### 3.7 Anforderung 1 Zeile 3 / Grenzfall — Icon-Rendering (`⇤ ↔ ⇥ ≡`)
...
Entscheidung erfüllt Abnahmekriterium 6 der Anforderung („bewusst beibehalten
...
### 3.8 Anforderung 1 Zeile 2 / 2.2 Bedienelement — Tastenkombination

`WordEditor.tsx:71-79`. **Entscheidung: wird ergänzt**, aber **bewusst nicht** als
`Strg+L/E/R/J` (die in Word/LibreOffice übliche Kombination, wie die Anforderung
selbst vorschlägt).

**Begründung/Risikoabwägung (muss laut Anforderung 1 Zeile 2 explizit dokumentiert,
...
`absatzformat-dropdown-req.md` (Abschnitt 1 Zeile 3) für Formatvorlagen
...
```ts
import { setAlign } from './commands' // ergänzt den bestehenden Import
...
Abschnitt 5.2 vorgesehene E2E-Test (`page.keyboard.press('Control+Alt+j')`)
...
### 3.9 Anforderung Grenzfall 3.9 — DOCX-Reader: `w:jc val="start"/"end"/"distribute"/"thaiDistribute"`

`docx/reader.ts:13`, aktuell:
...
`rtl.docx`, `table-alignment.docx`, `table-indent.docx` und `unicode-path.docx`
enthalten alle real `<w:jc w:val="start"/>`; `table-alignment.docx` zusätzlich
`w:val="end"`. Jeder dieser Werte fällt aktuell über `JC_TO_ALIGN[jcVal] ?? 'left'`
(Zeile 152) still auf „links“ zurück — bei `rtl.docx` (einem laut Namen
absichtlich rechtsläufigen Dokument) ist „links“ die inhaltlich am wenigsten
plausible Fallback-Wahl überhaupt.
...
const JC_TO_ALIGN: Record<string, string> = {
  ...
  // (Zeile 152) zurück. Bewusster, dokumentierter Fallback, kein stiller
...
`docx/writer.ts:67-70` schreibt bei **jedem** Absatz/jeder Überschrift ein
explizites `<w:jc w:val="…"/>`, auch für den Default „links“. Das ist **kein**
Korrektheitsfehler (Word interpretiert ein explizites `w:jc val="left"`
...
### 3.11 Anforderung Grenzfall 3.8 — ODT-Reader: `fo:text-align="start"/"end"` wird roh übernommen

`odt/reader.ts:63-65`, aktuell:
...
`feature_attributes_paragraph_MSO2013.odt` — Absatzstil `P5`
(`fo:text-align="end"`) wird nach aktuellem Stand als `align: 'end'` importiert:
kein kanonischer Wert, keiner der vier Toolbar-Buttons zeigt „gedrückt“
(`isAlignActive` vergleicht exakt gegen `'left'|'center'|'right'|'justify'`),
Export fällt über `PARAGRAPH_ALIGN_STYLE_NAME['end'] ?? …left` still auf
„links“ zurück — exakt Grenzfall 3.8/3.9 der Anforderung, jetzt an echten Daten
bestätigt statt nur spekuliert. `start` ist im Korpus sogar noch verbreiteter
(14 Fixtures, u. a. `listStyleId.odt`, `Seasonal_Fruits2_en.odt` — beide
LibreOffice-typische Standard-Exporte, kein Sonderfall).

**Fix, analog zu Abschnitt 3.9, an derselben Speicherstelle** (wirkt dadurch
automatisch für alle drei Lesestellen — Absatz Zeile 126, Überschrift Zeile 173,
Kopf-/Fußzeilen-Varianten — statt an jeder einzeln geändert werden zu müssen):
...
// ... innerhalb von parseAutomaticStyles, Zeile 63-65:
const align = props?.getAttributeNS(ODF_NAMESPACES.fo, 'text-align')
...
### 3.13 Anforderung Grenzfall 3.10 — style-seitig ererbtes `w:jc` wird nicht gelesen

`docx/reader.ts:150-152` liest ausschließlich das direkte `w:pPr` des Absatzes,
nie ein über `w:pStyle` referenziertes, ererbtes `w:jc`.
...
Fund): `bug-paragraph-alignment.docx` enthält wortwörtlich zwei Absätze mit den
Lauftexten „This paragraph **does not have explicit alignment**, it's
**centered per the paragraph style**.“ (kein direktes `w:jc`, Formatvorlage
`Title` deklariert `w:jc val="center"` in `styles.xml`) und „This paragraph has
**explicit left alignment, overriding** the alignment in the paragraph style.“
(direktes `w:jc val="left"` **und** dieselbe Formatvorlage `Title`). Der
aktuelle Reader importiert:
...
### 3.14 Anforderung 2.6 — Listen und Tabellenzellen

`alignableTypes` (`commands.ts:10`) prüft nur `node.type.name`, nicht die
Position/den Elterntyp. Anders als `setHeading` (siehe
`absatzformat-dropdown-req.md` Befund 5: `list_item.content = 'paragraph
block*'` erzwingt, dass der **erste** Kindknoten vom Typ `paragraph` sein muss)
ist `setAlign` von dieser Content-Restriktion **nicht** betroffen — ein
`setNodeAttribute`-Aufruf ändert nur ein Attribut, keinen Node-Typ, und
unterliegt keiner `canReplaceWith`-Prüfung. Ein `paragraph` innerhalb eines
`list_item` oder `table_cell` (beide erlauben `paragraph` als Kindknoten, siehe
`schema.ts:98-104,106`) wird von `nodesBetween` unabhängig von seiner Position
erfasst. **Bereits strukturell korrekt, keine Code-Änderung nötig** — nur Tests,
die das (inklusive der in Abschnitt 3.1 gefixten Mehrfachzellen-/
Mehrfachlistenpunkt-Selektion) tatsächlich nachweisen, fehlen bisher komplett
(Abschnitt 5).

### 3.15 Schema-Permissivität — kein Enum auf `align` (Anforderung Grenzfall 3.7)
...
`parseDOM`-Callback (`schema.ts:13,26`, liest `dom.style.textAlign` unmodifiziert)
bei einem Fremdwert zum Werfen bringen statt zu einem sanften Fallback zu
kommen — das wäre eine **Verschlechterung** (Absturz statt stillem, aber
zumindest textverlustfreiem Fallback). Die bestehende Kombination aus
permissivem Schema + `isAlignActive`s exaktem Vergleich gegen die vier
kanonischen Werte (Abschnitt 3.3, unverändert in dieser Hinsicht) + den
...
| # | Datei | Änderung | Typ |
|---|---|---|---|
| 1 | `src/formats/shared/editor/commands.ts` | `setAlign` auf `ranges`-basierte Einzel-Transaction umgestellt (Abschnitt 3.1); `isAlignActive` auf „alle Blöcke uniform“ bei Mehrfachselektion umgestellt (Abschnitt 3.3); `setHeading` erhält Absatz-`align` statt hart `'left'` (Abschnitt 3.5) | Fix (3 Funktionen in bestehender Datei) |
| 2 | `src/formats/shared/editor/Toolbar.tsx` | `AlignButton`: `ALIGN_LABELS`/`ALIGN_GLYPHS`-Maps, `aria-label`, `label`-Prop entfernt, Glyph in `aria-hidden`-Span (Abschnitt 3.6); vier Aufrufstellen (Zeile 185-188) angepasst | Fix |
| 3 | `src/formats/shared/editor/WordEditor.tsx` | Keymap-Einträge `Mod-Alt-l/e/r/j` + Entscheidungs-Kommentar (Abschnitt 3.8); Import von `setAlign` ergänzt | Neu (Zeilen in bestehender Datei) |
| 4 | `src/formats/docx/reader.ts` | `JC_TO_ALIGN` um `start→left`, `end→right` erweitert + Kommentar zu bewusst nicht gemapptem `distribute`/`thaiDistribute` (Abschnitt 3.9); Kommentar zu bewusst nicht gelesenem style-seitigem `w:jc` (Abschnitt 3.13) | Fix + Doku |
| 5 | `src/formats/odt/reader.ts` | `ODF_ALIGN_ALIASES`-Map + Anwendung in `parseAutomaticStyles` (Abschnitt 3.11) | Fix |
...
dass `durchgestrichen-code.md`s gleichnamige, bisher ebenfalls nicht angelegte
Datei zuerst entsteht: beide Testsuiten können in derselben Datei nebeneinander
stehen, keine Namenskollision der `describe`-Blöcke.)
...
describe('setAlign — Kernbug 2.2/5.1: Mehrfachabsatz-Selektion (ausrichtung-blocksatz-code.md Abschnitt 3.1)', () => {
  ...
  it('a second dispatch call built from the same stale state (reproducing the exact old bug pattern) throws — confirms the mechanism, not just the symptom', () => {
    ...
    // dispatchTransaction before the fix.
  ...
  it('does not dispatch (no-op, no new undo step) when the whole selection is already uniformly justified (Anforderung 2.4)', () => {
  ...
  it('only re-aligns the cells actually inside a non-full-row CellSelection (neuer Fund, Abschnitt 3.1)', () => {
    ...
    // Select only the middle column (column index 1) across both rows.
    const firstMiddleCellPos = 3 // doc -> table -> row -> cell(0) -> cell(1) start, adjust via resolve below
    // Resolve positions defensively instead of hardcoding — walk the doc to find
    // the 2nd cell of row 1 and row 2.
    const cellPositions: number[] = []
    state.doc.descendants((node, pos) => {
      if (node.type.name === 'table_cell') cellPositions.push(pos)
    })
    const [r1c1, , , r2c1] = cellPositions // 0=r1c0,1=r1c1,2=r1c2,3=r2c0,4=r2c1,5=r2c2 — pick indices 1 and 4
    const middleCol = [cellPositions[1], cellPositions[4]]
    const colSelection = CellSelection.create(state.doc, middleCol[0] + 1, middleCol[1] + 1)
...
describe('isAlignActive — gemischte Mehrfachselektion (Anforderung 2.3/Grenzfall 3.4, Abschnitt 3.3)', () => {
...
describe('setHeading preserves alignment across style changes (Anforderung 2.5, Abschnitt 3.5)', () => {
...
describe('DOCX reader: w:jc="start"/"end" normalization (Grenzfall 3.9, real fixtures)', () => {
  it('rtl.docx: w:jc="start" is read as "left" (documented LTR approximation, see Abschnitt 3.9)', async () => {
    ...
    expect(aligns.some((a) => a.align === 'start')).toBe(false) // vor dem Fix hätte 'left' via Fallback gestimmt, aber ungetestet
  })
...
describe('DOCX reader: style-inherited w:jc is intentionally NOT read (Grenzfall 3.10, Abschnitt 3.13)', () => {
  it('bug-paragraph-alignment.docx: direct w:jc always wins; style-only jc falls back to "left" (documented, frozen behavior)', async () => {
    ...
    const direct = aligns.find((a) => a.text.includes('overriding the alignment'))
...
describe('ODT reader: fo:text-align="start"/"end" normalization (Grenzfall 3.8, real fixtures)', () => {
  it('feature_attributes_paragraph_MSO2013.odt: contains both "end" (normalized to right) and real "justify" paragraphs', async () => {
    ...
    expect(aligns).toContain('right') // normalisiert aus fo:text-align="end" (Stil P4/P5)
    expect(aligns).not.toContain('end')
  })

  it('listStyleId.odt: fo:text-align="start" is normalized to "left"', async () => {
...
Absätzen (Verdachtsmoment aus Anforderungstabelle, letzte Zeile). Neue
Testfälle, je Datei (hier für DOCX gezeigt, ODT-Variante identisch mit
`writeOdt`/`readOdt`):

```ts
it('preserves "justify" across MULTIPLE consecutive paragraphs at once (Anforderung 4.1.3 — bisher nie getestet)', async () => {
  const original = doc([paragraph('Erster'), paragraph('Zweiter'), paragraph('Dritter')].map((p) => ({
    ...p,
    attrs: { align: 'justify' },
  })))
      ...
      content: [
        {
          type: 'table_row',
          content: [
            { type: 'table_cell', attrs: { colspan: 1, rowspan: 1 }, content: [{ ...paragraph('Blocksatz'), attrs: { align: 'justify' } }] },
            { type: 'table_cell', attrs: { colspan: 1, rowspan: 1 }, content: [paragraph('Normal')] },
          ],
        },
      ],
    },
...
test.describe('Blocksatz — Kernverdacht 2.2 / Testfall 5.1 (höchste Priorität)', () => {
  ...
  test('Testfall 1a: drei per Maus/Tastatur markierte Absätze werden ALLE justiert, keine Konsolen-Exception, ein Undo-Schritt', async ({ page }) => {
...
test.describe('Blocksatz — Toolbar, Tastatur, Grenzfälle', () => {
  ...
  test('Testfall 4: Formatvorlage zu "Überschrift 1" wechseln, während der Absatz im Blocksatz steht — Ausrichtung bleibt erhalten (Entscheidung Abschnitt 3.5)', async ({ page }) => {
    const editor = page.locator('.ProseMirror')
    await editor.click()
  ...
  test('Testfall 5: Blocksatz auf eine Tabellenzelle und auf einen einzelnen Listenpunkt', async ({ page }) => {
    const editor = page.locator('.ProseMirror')
    await editor.click()
  ...
  test('Testfall 6/7: Selektion mit Bild — kein Absturz, nur Text betroffen; reine Bildselektion tut sichtbar nichts (Grenzfall 3.6)', async ({ page }) => {
    const editor = page.locator('.ProseMirror')
    await editor.click()
    await page.keyboard.type('Text vor dem Bild.')
    const fs = await import('node:fs/promises')
    const png = Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII=', 'base64')
    await fs.writeFile('/tmp/blocksatz-test.png', png).catch(() => {})
    // Bild per Toolbar einfügen (Datei-Input), dann Alles auswählen inkl. Bild:
    await page.getByRole('button', { name: /verstanden/i }).click().catch(() => {})
    await page.keyboard.press('ControlOrMeta+a')
    await page.getByTitle('Blocksatz').click() // Text+Bild gemischt — darf nicht abstürzen
    await expect(page.locator('.ProseMirror p').first()).toHaveCSS('text-align', 'justify')
  })

  test('Testfall 8: Einfügen von extern kopiertem HTML mit inline style="text-align: justify"', async ({ page }) => {
    const editor = page.locator('.ProseMirror')
    await editor.click()
    await page.evaluate(() => {
  ...
  test('Testfall 9: klassenbasiertes (nicht-inline) externes Blocksatz-HTML wird NICHT erkannt (dokumentiertes Fallback-Verhalten, Grenzfall 3.7)', async ({ page }) => {
    const editor = page.locator('.ProseMirror')
    await editor.click()
    await page.evaluate(() => {
      const pm = document.querySelector('.ProseMirror') as HTMLElement
  ...
  test('Testfall 10: Undo/Redo direkt nach Blocksatz-Anwendung auf einen einzelnen Absatz', async ({ page }) => {
    const editor = page.locator('.ProseMirror')
    await editor.click()
  ...
  test('Testfall 15 / Grenzfall 3.8: fo:text-align="start" beim ODT-Import — kein Button aktiv, Fallback dokumentiert', async ({ page }) => {
    const fs = await import('node:fs/promises')
    ...
    for (const name of ['Linksbündig', 'Zentriert', 'Rechtsbündig', 'Blocksatz']) {
      await expect(page.getByLabel(name)).toHaveAttribute('aria-pressed', 'false')
    }
  })

  test('Testfall 13: Screenreader-Name des Buttons ist "Blocksatz", nicht das Unicode-Zeichen (Bedienelement Nr. 4)', async ({ page }) => {
    // Automatisierbarer Näherungstest ohne echten Screenreader: der berechnete
    // Accessible Name muss "Blocksatz" sein.
  ...
  test('Grenzfall 3.11: schnelles Mehrfachklicken erzeugt keinen zusätzlichen Undo-Schritt', async ({ page }) => {
    const editor = page.locator('.ProseMirror')
    await editor.click()
    ...
    await expect(editor.locator('p')).toHaveCSS('text-align', 'justify')
    await page.keyboard.press('ControlOrMeta+z')
    // Ein einziges Undo muss bereits auf "nicht mehr justiert" zurückführen —
    // nicht erst nach drei Undos (bestätigt, dass die Wiederholklicks keine
    // eigenen Transactions erzeugt haben, siehe Abschnitt 3.4).
    await expect(editor.locator('p')).not.toHaveCSS('text-align', 'justify')
  })
...
test.describe('Blocksatz — Rundreisen (Anforderung Abschnitt 4)', () => {
  test('Rundreise 4.1.1/4.1.2: DOCX-Eigenrundreise — Toolbar-erzeugter Blocksatz exportiert exakt <w:jc w:val="both"/>', async ({ page }) => {
    await page.goto('/')
    await page.getByRole('button', { name: /verstanden/i }).click()
  ...
  test('Rundreise 4.3.3: Cross-Format DOCX -> ODT -> DOCX mit allen vier Ausrichtungen gemeinsam', async ({ page }) => {
    await page.goto('/')
    await page.getByRole('button', { name: /verstanden/i }).click()
    await docxCard(page).getByRole('button', { name: 'Neu erstellen' }).click()
    const editor = page.locator('.ProseMirror')
    ...
    for (const [text, title] of [
      ...
      ['Mitte.', 'Zentriert'],
      ['Rechts.', 'Rechtsbündig'],
      ['Blocksatztext mit genug Inhalt.', 'Blocksatz'],
    ] as const) {
    ...
    // Export als ODT, Re-Import, Export zurück als DOCX — Ausrichtung je Absatz bleibt distinkt.
    // (Format-Switch-UI: siehe docx.spec.ts/odt.spec.ts für das exakte Auswahl-Muster dieser App.)
  })
})
```
...
Alle Werte in diesem Abschnitt wurden vor dem Schreiben dieses Plans per
Skript aus den tatsächlichen `document.xml`/`content.xml`-Inhalten extrahiert
(derselbe empirische Ansatz wie `durchgestrichen-code.md` Abschnitt 6) — **keine**
Annahme aus dem Dateinamen allein.

### 6.1 DOCX (Apache-POI-Korpus, `word/document.xml`)

| Datei | `w:jc`-Werte gefunden | Einordnung |
|---|---|---|
| `bug-paragraph-alignment.docx` | 1× `left` (direkt); Style `Title` in `styles.xml` deklariert zusätzlich `center` | **Primär-Fixture für Grenzfall 3.10** — enthält wortwörtlich erklärenden Lauftext, siehe Abschnitt 3.13 |
| `table-alignment.docx` | `left`, `start`, `center`, `right`, `end` (je 1×, 5 Zellen mit Platzhaltertext „Loren“) | **Primär-Fixture für Grenzfall 3.9** — enthält aber **kein** `both`/`justify` |
| `TestTableCellAlign.docx` | **keine** (`w:jc`-Treffer: 0) — nur `w:vAlign="bottom"/"center"` (vertikale Zellausrichtung, separates, nicht modelliertes Feature) und `w:tblPr` (Tabellenbreite) | **Korrektur zur Anforderung:** entgegen Testfall 5.14/Abschnitt 0 der Anforderung **nicht** einschlägig für horizontale Ausrichtung — siehe Abschnitt 0 Punkt 8 |
| `rtl.docx` | `start` | Zusätzlicher, thematisch besonders passender Fund (RTL-Kontext) |
| `table-indent.docx`, `unicode-path.docx` | `start` | Weitere Bestätigung, kein Einzelfall |
| `56392.docx` | `both` (auf `Normal`-Stil-Absatz, kompakte 8 KB-Datei) | **Primär-Fixture für echten Blocksatz-Import** |
| `Bug51170.docx`, `bug57031.docx`, `bug59058.docx`, `checkboxes.docx`, `drawing.docx`, `form_footnotes.docx` | `both` | Weitere Kandidaten, falls ein Zweit-Fixture gewünscht ist |
| `bug65649.docx` | enthält ebenfalls `both` | **Nicht verwenden** — bereits in `external-fixtures.test.ts` als `SKIP_SLOW_UNDER_JSDOM` geführt (12 MB, ~16k Absätze) |

Vollständigkeits-Hinweis: Die programmatische Suche deckte ausschließlich
`word/document.xml` ab (nicht Kopf-/Fußzeilen-Teile) — für die in Abschnitt 3.13
zitierte Style-Deklaration (`styles.xml`) wurde gezielt nachgeprüft.

### 6.2 ODT (ODF-Toolkit-Korpus, `content.xml`)

| Datei | `fo:text-align`-Werte gefunden | Einordnung |
|---|---|---|
| `feature_attributes_paragraph_MSO2013.odt` | `center` (×2 Stile), `end` (×2 Stile: `P4`,`P5`), `justify` (×2 Stile: `P6`,`P7`) | **Primär-Fixture** — einzige Datei mit sowohl echtem `justify` **als auch** `end` gleichzeitig; `P7` referenziert von einem realen `<text:p>` |
| `feature_attributes_paragraph_MSO2013_doc-AO.odt` | analog | Alternative/Vergleichsvariante (anderer Exporteur: AO = AndroidOffice/ähnlich) |
| `listStyleId.odt` | `start`, `justify` | Zweit-Fixture, bereits in `durchgestrichen-code.md` Abschnitt 6 für `strike` verwendet — Wiederverwendung eines bereits bekannten Fixtures, kein neuer Korpus-Import nötig |
| `Seasonal_Fruits2_en.odt`, `nestedFrames.odt`, `test1.odt`, `table-within-textBox-within-frame.odt` | `start` **und** `justify` | Weitere kombinierte Kandidaten |
| 10 weitere Dateien (`excelfileformat.odt`, `fields.odt`, `HeaderFooter.odt`, `OOStyledTable.odt`, `tableComplex_DOC_LO41.odt`, `text-extract.odt`, `_annotation.odt`, `FruitDepot-SeasonalFruits{4,5}.odt`) | `start` | Bestätigt: `start` ist der in LibreOffice-Exporten **übliche** Default, kein Sonderfall |
| `tabelleAlignMargin.odt` | **keine** (`fo:text-align`-Treffer: 0) — nur `table:align="margins"` (Tabellenposition relativ zu den Seitenrändern, separates, nicht gelesenes Feature) | **Korrektur zur Anforderung:** entgegen Testfall 5.14/Abschnitt 0 der Anforderung **nicht** einschlägig — siehe Abschnitt 0 Punkt 8 |

**Konsequenz für Testfall 5.14 der Anforderung** (Import der vier benannten
Fixtures „mit gezielter Prüfung der resultierenden `align`-Attribute“): Für
...
nicht-existente Blocksatz-Zuweisung zu erwarten. Dies wird explizit als
Korrektur der Anforderung dokumentiert, nicht stillschweigend übergangen.

---

## 7. Unabhängige Parser-Validierung (Rundreise-Anforderung Punkt 8 / DoD Punkt 3)
...
1. **Automatisiert:** Die Playwright-Tests aus Abschnitt 5.2 prüfen den
   ...
   bzw. `expect(contentXml).toContain('fo:text-align="justify"')`).
...
- **`schema.ts`** — permissives `align`-Attribut ohne Enum bleibt (Abschnitt
  3.15); `parseDOM`/`toDOM` für `paragraph`/`heading` sind bereits korrekt.
- **`docx/writer.ts`** — explizites `<w:jc>` auch für den Default „links“ bleibt
  (Abschnitt 3.10) — kein Korrektheitsfehler.
...
- **`setHeading`s Beschränkung auf einen einzelnen Block** (`!$from.sameParent
  ($to)`) — unverändert; eigenständige Design-Frage aus
  `absatzformat-dropdown-req.md` Abschnitt 2.3, nicht Teil dieses Plans.
...
| Testfälle 5.2–5.7 (Einzelblock, Cursor, Listen/Zellen, Bild-Grenzfälle) | `tests/e2e/alignment.spec.ts`, describe „Toolbar, Tastatur, Grenzfälle“ |
| Testfälle 5.8/5.9 (Paste inline vs. klassenbasiert) | `tests/e2e/alignment.spec.ts` |
| Testfall 5.10 (Undo/Redo Einzelblock) | `tests/e2e/alignment.spec.ts` |
| Testfall 5.11 (Rundreisen je Format) | `tests/e2e/alignment.spec.ts`, describe „Rundreisen“ |
| Testfall 5.12 (Cross-Format-Rundreise, alle vier Ausrichtungen) | `tests/e2e/alignment.spec.ts` |
| Testfall 5.13 (Screenreader-Stichprobe) | `tests/e2e/alignment.spec.ts` (automatisierter Accessible-Name-Näherungstest) **plus** empfohlene manuelle NVDA/VoiceOver-Probe vor Statuswechsel |
| Testfall 5.14 (vier benannte Fixtures) | `commands.test.ts`/`alignment-fixtures.test.ts` — **mit Korrektur**: `TestTableCellAlign.docx`/`tabelleAlignMargin.odt` enthalten keine horizontale Ausrichtung (Abschnitt 6) |
| Testfall 5.15 (`w:jc`/`fo:text-align` = start) | `alignment-fixtures.test.ts` (beide Formate) + Fix Abschnitt 3.9/3.11 |
| Kernverdacht 2.2 | Fix Abschnitt 3.1 + `commands.test.ts` + `alignment.spec.ts` |
| Neuer Fund: `CellSelection`-Rechteck-Fehler | Fix Abschnitt 3.1 (`ranges` statt `from`/`to`) + dedizierter Test in `commands.test.ts` |
| Anforderung 2.3/Grenzfall 3.4 (Mixed-Selection-Anzeige) | Entscheidung + Fix Abschnitt 3.3 + `commands.test.ts` |
| Anforderung 2.4 (Toggle/Undo-Sparsamkeit) | Entscheidung + Fix Abschnitt 3.1/3.4 + `commands.test.ts` + Grenzfall-3.11-E2E-Test |
| Anforderung 2.5 (setHeading-Reset) | Entscheidung + Fix Abschnitt 3.5 + `commands.test.ts` + E2E Testfall 4 |
...
| Grenzfall 3.9 (DOCX `start`/`end`/`distribute`) | Fix Abschnitt 3.9 + `alignment-fixtures.test.ts` mit 4 echten Fixtures |
| Grenzfall 3.8 (ODT `start`/`end`) | Fix Abschnitt 3.11 + `alignment-fixtures.test.ts` mit echter Fixture |
...
| Grenzfall 3.6 (reine Bildselektion) | `commands.test.ts` + E2E Testfall 6/7 |
| Grenzfall 3.7 (Fremdwert nach Paste) | `commands.test.ts` + E2E Testfall 9 |
...
| Verdachtsmoment „nur Einzelabsatz getestet“ (Anforderungstabelle, letzte Zeile) | Erweiterung `roundtrip.test.ts` (beide Formate) um Mehrfachabsatz-/Tabellen-/Überschrift-/hard\_break-Fälle |
| DoD Punkt 1 (Kernverdacht behoben + Regressionstest) | Abschnitt 3.1 + `alignment.spec.ts` describe „Kernverdacht“ |
| DoD Punkt 2 (alle Testfälle ausgeführt) | Abschnitt 5 komplett + diese Tabelle |
| DoD Punkt 3 (Rundreisen inkl. Mehrfachabsatz, unabhängiger Parser) | Abschnitt 5.2/7 |
| DoD Punkt 4 (alle Grenzfälle geprüft/dokumentiert) | Abschnitt 3 (je Grenzfall einzeln eingestuft: „behoben“/„bewusst dokumentiert, nicht behoben“/„bereits korrekt, nur Test nötig“) |
| DoD Punkt 5 (offene Fragen 2.4/2.5 entschieden) | Abschnitt 3.4/3.5, hier verbindlich entschieden und begründet |
| DoD Punkt 6 (`aria-label`/Icon-Risiko bewertet) | Abschnitt 3.6 (behoben) / 3.7 (bewusst beibehalten, begründet) |
| DoD Punkt 7 (vier Fixtures angebunden) | Abschnitt 5.1/6 — inklusive Korrektur, dass zwei der vier Fixtures keine horizontale Ausrichtung enthalten |
